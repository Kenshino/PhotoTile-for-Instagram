...
//////////////////////////////////////////////////////////////////////////////////////
	...
	function cleanCache() {
		...
		if (file_exists($cleaning_info))  {
			$cache_time = intval( file_get_contents ( $cleaning_info ) ) + intval( $this->get_private( 'cleaningInterval' ) ); //Last update time of the cache cleaning
			$time = time(); //Current Time
			$expiry_time = (int)$time; //Expiry time for the cache
			if ((int)$cache_time < (int)$expiry_time){ //Compare last updated and current time
				// Clean old files
						...
						if($file != "." && $file != "..") {
							...
							elseif(file_exists($dir.$file)) {
								...
								if( $file_type && $file_key && 'info' == $file_type){
									...
									if (file_exists($filename_cache) && file_exists($filename_info)) {
										$cache_time = intval( file_get_contents ($filename_info) ) + intval( $this->get_private( 'cleaningInterval' ) ); //Last update time of the cache file
										$expiry_time = (int)$time; //Expiry time for the cache
										if ((int)$cache_time < (int)$expiry_time) {//Compare last updated and current time
...
class PhotoTileForInstagramBot extends PhotoTileForInstagramBotTertiary{
	...
	function display_vertical(){
		...
		$css = "margin:0;padding:0;max-width:100%;";
	...
	function display_cascade(){
		...
		for($col = 0; $col<$opts['style_column_number'];$col++){
			...
			$this->add('<div class="AlpinePhotoTiles_cascade_column_inner" style="display:block;margin:0;overflow:hidden;">');
